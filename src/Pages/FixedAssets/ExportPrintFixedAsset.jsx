    ...
    try {
      ...
      const newObj = res.flatMap((item) => {
        return {
          ...
          Company: item?.one_charging?.company_name || item?.company.company_name,
          "Business Unit Code": item?.one_charging?.business_unit_code || item?.business_unit.business_unit_code,
          "Business Unit": item?.one_charging?.business_unit_name || item?.business_unit.business_unit_name,
          "Department Code": item?.one_charging?.department_code || item?.department.department_code,
          Department: item?.one_charging?.department_name || item?.department.department_name,
          "Unit Code": item?.one_charging?.unit_code || item?.unit.unit_code,
          Unit: item?.one_charging?.unit_name || item?.unit.unit_name,
          "Subunit Code": item?.one_charging?.codsubunit_codee || item?.subunit.subunit_code,
          Subunit: item?.one_charging?.subunit_name || item?.subunit.subunit_name,
          "Location Code": item?.one_charging?.location_code || item?.location.location_code,
          Location: item?.one_charging?.location_name || item?.location.location_name,
